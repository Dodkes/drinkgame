//First category questions/tasks
const questions = [
    'Všetci chlapci si vypijú',
    'Čoho sa najviac bojíš',
    'Povedz vtip',
    'Čo ťa najviac vytočí?',
    'V poradí 3. človek na ľavo od teba pije s tebou',
    'Všetky ženy si vypijú',
    'Vypi, alebo vyber hráča, ktorý vypije za teba',
    'Vyber si človeka s ktorým vypiješ',
    'Čo by si na sebe zmenil/a?',
    'S ktorým hráčom by si sa rád/a ocitol/la na opustenom ostrove a prečo?',
    'Vymysli nový produkt a pomenuj ho',
    'Vyber si 1 hráča a povedz prečo by si s ním šiel na rande',
    'Vyber si 1 hráča a prirovnaj ho k nejakému superhrdinovi, odôvodni to',
    'S ktorou slávnou osobnosťou by si šiel/šla na rande?',
    'Ak ti padlo nepárne číslo, piješ',
    'Vymenuj nejaký svoj zlozvyk',
    'Vyber si človeka a povedz čo ti na ňom prekáža',
    'Vymysli názov knihy, ktorá by vystihovala tvoj život',
    'Kde budeš o 10 rokov?',
    'Priznaj nejakú tvoju slabosť',
]

const challengeTasks = [
    'Vymenuj všetkých 6 šachových figúriek',
    'Vyzuj si oboje ponožky',
    'Vymenuj akékoľvek 4 mestá českej republiky',
    'Vymenuj akékoľvek 4 mimoeurópske hlavné mestá',
    'Prines kovový predmet',
    'Vymenuj 5 mien na písmeno P',
    'Postni akúkoľvek fotku na instagram story',
    'Prines akýkoľvek žltý predmet',
    'Prines drevený predmet',
    'Vymysli báseň, ktorá má aspoň 4 verše',
    'Vypi',
    'Sprav 5 drepov',
    'Sprav si selfie na svojom mobile',
]

const duel = [
    'Strihanie',
    'Kto skôr žmurkne',
    'Kto sa skôr zasmeje',
    'Palcová hra',
    'Pretláčanie rukou',
    'Otvorenie SMS aplikácie na mobile so zatvorenými očami',
]

const tasks = [
    'Vypi',
    'Vyzleč si čokoľvek, čo máš na sebe',
    'Vyber si hráča, s ktorým si vymeníš čokoľvek čo máte na sebe 1 za 1',
    'Prehovor kanibala, aby ťa nezjedol, použi aspoň 3 vety',
    'Vezmi si akýkoľvek predmet, ktorý budeš nasledujúce 2 kolá po celý čas držať oboma rukami',
    'Zahraj sa na plačúce dieťa',
    'Zahryzni si do jazyka a odrecituj abecedu',
    'Olíž si lakeť',
    'Vydrž v póze lastovičky aspoň 5 sekúnd',
    'Nechaj sa niekym štekliť po dobu 7 sekúnd',
    'Rozosmej iného hráča',
    'Nakresli 1 rukou trojuholník a súčasne 2. rukou kruh',
    'Osoba po tvojej ľavici ti dá facku',
    'Vymenuj 5 zvierat, ktoré by si nenašiel v ZOO',
    'Vymenuj 3 veci, ktoré by pokazili rande',
]

const facts = [
    'Tvoja najtrápnejšia historka, akú si pamätáš',
    'Kedy si sa prvykrát v živote opil?',
    'Skúsil si marihuanu? V koľkých rokoch prvý krát?',
    'V koľkých rokoch si sa prvý krát opil?',
    'Máš príležitosť vybrať si hráča a prezradiť o ňom niečo trápne z minulosti',
    'Už si niekedy čúral/la v bazéne?',
    'Koľko frajeriek/ov si mal/a vo svojom živote?',
    'Popíš aký priestupok/trestný čin si naposledy spravil/a',
    'Kedy si sa najviac opil/a? Popíš to',
    'Akú najtrápnejšiu vec si kupoval/a?',
    'Na akom najbizárnejšom mieste si zaspal/a?',
    'Už si niekedy niečo ukradol/la? Čo to bolo a prečo?',
    'Akú najdlhšiu dobu si sa nesprchoval/la?',
]