...
var oponent;
let addPoints = true;
const questionsToPlay = isAdultMode ? questions18Plus : questions;
const tasksToPlay = isAdultMode ? tasks18Plus : tasks;
const duelToPlay = isAdultMode ? duel18Plus : duel;
...
function firstCategory() {
  ...
  easyTask.innerText =
    questionsToPlay[Math.floor(Math.random() * questionsToPlay.length)];
  drinkAudio();
...
function updateDisplay() {
  ...
  rewrite.innerText =
    players[playerOrder].playerName +
    " : " +
    players[playerOrder].playerPoints +
    "b";
}
...
function task() {
  ...
  cardsText.innerText =
    tasksToPlay[Math.floor(Math.random() * tasksToPlay.length)];
  taskAudio();
}
...
function duelGame() {
  ...
  cardsText.innerText =
    duelToPlay[Math.floor(Math.random() * duelToPlay.length)] +
    " s hráčom " +
    players[oponent].playerName;
  duelAudio();
}
...
function updateOponentDisplay() {
  ...
  rewriteOponent.innerText =
    players[oponent].playerName + " : " + players[oponent].playerPoints + "b";
}
